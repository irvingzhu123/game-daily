...
# 2. 检查是否有新文章
RESULT=$(cat /tmp/today_articles.json 2>/dev/null)
if [ "$RESULT" = "NO_NEW_ARTICLES" ] || [ -z "$RESULT" ]; then
  echo "$(date '+%Y-%m-%d %H:%M:%S') [deploy] 无新增文章" >> /tmp/game_daily.log
  exit 0
fi

ARTICLE_COUNT=$(echo "$RESULT" | python3 -c "import sys,json; print(len(json.load(sys.stdin)))" 2>/dev/null || echo "?")
...
cd "$SITE_DIR"
git add -A
git commit -m "📊 游戏行业日报 $TODAY" --allow-empty 2>/dev/null
